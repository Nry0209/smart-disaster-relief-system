...
import { createResourceRequest, fetchPartners } from "../services/workflowService";
...
const WAREHOUSES = ["Colombo Central Warehouse", "Kandy Regional Warehouse"];
...
export default function ResourceRequestInlineForm({ onSubmitted }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        setLoading(true);
        const [partnerList, inventory] = await Promise.all([
  ...
  const inventoryCategories = useMemo(() => {
    return [...new Set(selectableInventoryItems.map((item) => item.category))].sort((a, b) => a.localeCompare(b));
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const requestIdValue = resourceRequest?._id || resourceRequest?.id;
      const inboxPath = requestIdValue ? `/ngo-inbox?requestId=${requestIdValue}` : "/ngo-inbox";
      ...
      if (typeof onSubmitted === "function") {
        onSubmitted(resourceRequest);
      }

      if (user?.role === "ngo_partner" && requestIdValue) {
        setTimeout(() => {
          window.location.assign(inboxPath);
        }, 1200);
      }
    } catch (requestError) {
      setError(requestError.message || "Failed to submit resource request. Please try again.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <h2 className="text-xl font-semibold text-slate-900">Resource Request</h2>
          <p className="text-sm text-slate-500">Submit a new request without leaving the NGO page.</p>
        </div>
      </div>
          ...
          <>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="form-group">
                ...
                <select value={form.requestType} onChange={(e) => updateField("requestType", e.target.value)} required>
